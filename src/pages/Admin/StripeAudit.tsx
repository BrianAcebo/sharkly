...
export default function StripeAudit() {
  ...
  const handleSearch = async () => {
    ...
    try {
      const headers = await getAuthHeaders();
      const resp = await api.get(`/api/billing/admin/refunds/lookup?query=${encodeURIComponent(searchQuery.trim())}`, { headers });
  ...
  const loadAuditData = async (orgId: string) => {
    ...
    try {
      ...
      const { data: webhookEvents } = await supabase
        ...
        .select('id, stripe_event_id, event_type, status, processing_error, received_at, processed_at')
        .eq('org_id', orgId)
      ...
      const { data: snapshots } = await supabase
        ...
        .select('id, stripe_subscription_id, event_type, status, current_period_start, current_period_end, cancel_at_period_end, created_at')
        .eq('org_id', orgId)
      ...
      const { data: invoices } = await supabase
        ...
        .select('id, stripe_invoice_id, status, amount_due, amount_paid, invoice_pdf, hosted_invoice_url, billing_reason, created_at')
        .eq('org_id', orgId)
        .order('created_at', { ascending: false })
        .limit(20);
      ...
      const { data: charges } = await supabase
        .from('stripe_charges')
        .select('id, stripe_charge_id, amount, amount_refunded, status, paid, refunded, receipt_url, card_brand, card_last4, stripe_created_at')
      ...
      const { data: failures } = await supabase
        ...
        .select('id, failure_code, failure_message, decline_code, next_action, attempt_count, created_at')
        .eq('organization_id', orgId)
        .order('created_at', { ascending: false })
        .limit(20);
  ...
  const handleSyncFromStripe = async () => {
    ...
    try {
      const headers = await getAuthHeaders();
      const resp = await api.post('/api/billing/admin/stripe/sync', { orgId: selectedOrgId }, { headers });
      const result = await resp.json();
      ...
      toast.success(`Synced: ${result.results?.invoices || 0} invoices, ${result.results?.charges || 0} charges`);
  ...
  const filteredWebhookEvents = auditData?.webhook_events.filter(e =>
    webhookTypeFilter === 'all' || e.event_type === webhookTypeFilter
  ) || [];
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        {auditData && !loading && (
          <div className="space-y-6">
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                  ...
                  <div>
                    ...
                    <Badge variant={auditData.organization.stripe_status === 'active' ? 'default' : 'secondary'}>
                      {auditData.organization.stripe_status || 'No subscription'}
                    </Badge>
                  </div>
                  <div>
                    <div className="text-xs text-gray-500">Customer ID</div>
                    <div className="font-mono text-xs">{auditData.organization.stripe_customer_id || 'N/A'}</div>
                  </div>
                  <div>
                    <div className="text-xs text-gray-500">Created</div>
                    <div className="font-medium">{formatDate(auditData.organization.created_at)}</div>
                  </div>
                </div>

                {/* Stripe Links & Actions */}
                {(auditData.organization.stripe_customer_id || auditData.organization.stripe_subscription_id) && (
                  <div className="flex flex-wrap gap-2 pt-3 border-t">
                    {auditData.organization.stripe_customer_id && (
                      <Button 
                        variant="outline" 
                        size="sm" 
                        onClick={() => openStripeCustomer(auditData.organization.stripe_customer_id!)}
                      >
                    ...
                    {auditData.organization.stripe_subscription_id && (
                      <Button 
                        variant="outline" 
                        size="sm" 
                        onClick={() => openStripeSubscription(auditData.organization.stripe_subscription_id!)}
                      >
                        <ExternalLink className="w-3 h-3 mr-1" />
                        View Subscription
                      </Button>
            ...
            <Card>
              <CardHeader className="cursor-pointer" onClick={() => setLedgerExpanded(!ledgerExpanded)}>
                <div className="flex items-center justify-between">
                  <CardTitle className="text-base flex items-center gap-2">
            ...
            {auditData.payment_failures.length > 0 && (
              <Card>
                <CardHeader className="cursor-pointer" onClick={() => setFailuresExpanded(!failuresExpanded)}>
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-base flex items-center gap-2">
                      <AlertTriangle className="w-4 h-4 text-red-500" />
                      Payment Failures ({auditData.payment_failures.length})
                    </CardTitle>
                    {failuresExpanded ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                  </div>
            ...
            {/* Invoices */}
            <Card>
              <CardHeader className="cursor-pointer" onClick={() => setInvoicesExpanded(!invoicesExpanded)}>
                <div className="flex items-center justify-between">
                  <CardTitle className="text-base flex items-center gap-2">
                      ...
                      {auditData.invoices.map((inv) => (
                        <div key={inv.id} className="flex items-center justify-between p-2 rounded bg-gray-50 dark:bg-gray-800">
                          <div>
                            ...
                            <div className="text-xs text-gray-500">{inv.billing_reason} • {formatDate(inv.created_at)}</div>
                          </div>
                          <div className="flex items-center gap-2">
                            ...
                            {inv.invoice_pdf && (
                              <Button variant="ghost" size="sm" onClick={() => window.open(inv.invoice_pdf!, '_blank')}>
                                <FileText className="w-4 h-4" />
                              </Button>
                            )}
                            {inv.hosted_invoice_url && (
                              <Button variant="ghost" size="sm" onClick={() => window.open(inv.hosted_invoice_url!, '_blank')}>
            ...
            {/* Charges */}
            <Card>
              <CardHeader className="cursor-pointer" onClick={() => setChargesExpanded(!chargesExpanded)}>
                <div className="flex items-center justify-between">
                  <CardTitle className="text-base flex items-center gap-2">
                    <CreditCard className="w-4 h-4" />
                    Charges ({auditData.charges.length})
                  </CardTitle>
                  {chargesExpanded ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                </div>
              </CardHeader>
              {chargesExpanded && (
                <CardContent>
                  {auditData.charges.length === 0 ? (
                    ...
                    <div className="space-y-2 max-h-80 overflow-y-auto">
                      {auditData.charges.map((charge) => (
                        <div key={charge.id} className="flex items-center justify-between p-2 rounded bg-gray-50 dark:bg-gray-800">
                          <div>
                            <div className="flex items-center gap-2">
                              ...
                              {charge.refunded && (
                                <Badge variant="outline" className="text-xs">Refunded: {formatCents(charge.amount_refunded)}</Badge>
                              )}
                            </div>
                            <div className="text-xs text-gray-500">
                              {charge.card_brand} •••• {charge.card_last4} • {formatDate(charge.stripe_created_at)}
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                            <Badge variant={charge.status === 'succeeded' ? 'default' : 'destructive'}>
                              {charge.status}
                            </Badge>
                            {charge.receipt_url && (
                              <Button variant="ghost" size="sm" onClick={() => window.open(charge.receipt_url!, '_blank')}>
            ...
            {/* Webhook Events */}
            <Card>
              <CardHeader className="cursor-pointer" onClick={() => setWebhooksExpanded(!webhooksExpanded)}>
                <div className="flex items-center justify-between">
                  <CardTitle className="text-base flex items-center gap-2">
                    <Webhook className="w-4 h-4" />
                    Webhook Events ({auditData.webhook_events.length})
                  </CardTitle>
                  {webhooksExpanded ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                </div>
                      ...
                      {filteredWebhookEvents.map((event) => (
                        <div key={event.id} className="flex items-center justify-between p-2 rounded bg-gray-50 dark:bg-gray-800">
                          <div>
                            ...
                            <div className="text-xs text-gray-500 font-mono">{event.stripe_event_id}</div>
                            <div className="text-xs text-gray-500">{formatDate(event.received_at)}</div>
                            {event.processing_error && (
                              <div className="text-xs text-red-500 mt-1">{event.processing_error}</div>
                            )}
                          </div>
                          <Badge variant={
            ...
            <Card>
              <CardHeader className="cursor-pointer" onClick={() => setSnapshotsExpanded(!snapshotsExpanded)}>
                <div className="flex items-center justify-between">
                  <CardTitle className="text-base flex items-center gap-2">
                    <History className="w-4 h-4" />
                    Subscription Snapshots ({auditData.subscription_snapshots.length})
                  </CardTitle>
                  {snapshotsExpanded ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                </div>
                  ...
                  {auditData.subscription_snapshots.length === 0 ? (
                    ...
                    <div className="space-y-2 max-h-80 overflow-y-auto">
                      {auditData.subscription_snapshots.map((snap) => (
                        <div key={snap.id} className="flex items-center justify-between p-2 rounded bg-gray-50 dark:bg-gray-800">
                          <div>
                            <div className="flex items-center gap-2">
                              <Badge variant="outline" className="text-xs">{snap.event_type}</Badge>
                              <Badge variant={snap.status === 'active' ? 'default' : 'secondary'}>{snap.status}</Badge>
                            </div>
                            <div className="text-xs text-gray-500 mt-1">
                              Period: {snap.current_period_start ? formatDate(snap.current_period_start) : 'N/A'} → {snap.current_period_end ? formatDate(snap.current_period_end) : 'N/A'}
                            </div>
                            <div className="text-xs text-gray-500">{formatDate(snap.created_at)}</div>
                          </div>
                          {snap.cancel_at_period_end && (
                            <Badge variant="destructive" className="text-xs">Cancel at period end</Badge>
                          )}
                        </div>
                      ))}
                    </div>
