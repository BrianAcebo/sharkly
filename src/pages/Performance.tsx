...
export default function Performance() {
	...
	// GSC is part of V1, no tier gating needed
	...
	return (
		<>
			...
			<div className="space-y-6">
				...
				<AIInsightBlock
					...
					message={
						hasGscData
							...
							: 'Connect Google Search Console to unlock performance insights, re-optimization suggestions, and Navboost CTR trends. Your published content will appear here once GSC is linked.'
					}
				/>
					...
					<div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
						<div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-900">
							<div className="text-center">
								<p className="text-sm text-gray-600 dark:text-gray-400">Connect GSC to see data</p>
							</div>
						</div>
						<div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-900">
							<div className="text-center">
				...
				{/* Re-Optimization Queue — V1 Section 7.8 "Ready to Promote" */}
				<div className="mt-6 overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900">
					...
					<div className="p-6">
						{gscConnected ? (
							hasGscData &&
							topPages.some((p) => p.position >= 4 && p.position <= 15 && p.impressions >= 500) ? (
								<table className="w-full">
									...
									<tbody>
										{topPages
											.filter((p) => p.position >= 4 && p.position <= 15 && p.impressions >= 500)
											.slice(0, 5)
											.map((page) => (
												<tr
													key={page.page}
												...
												>
													<td className="truncate py-4 text-sm text-gray-900 dark:text-white">
														{page.page}
													</td>
				...
				{/* Navboost Momentum Panel — V1 Section 7.8 */}
				<div className="mt-6 overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900">
					<div className="flex items-center justify-between border-b border-gray-200 p-5 dark:border-gray-700">
						<h3 className="font-montserrat text-base font-bold text-gray-900 dark:text-white">
							Navboost Momentum
						...
						<span
							...
							title="Google's click signal compounds over 13 months. Improving CTR today builds ranking power."
						>
							13-week CTR trend
						</span>
					</div>
					<div className="p-6">
						{gscConnected ? (
							hasGscData && topQueries.length > 0 ? (
								<div className="space-y-3">
									<p className="text-[13px] text-gray-600 dark:text-gray-400">
										Top keywords by CTR (13-week trend data coming from daily sync):
									</p>
									<div className="space-y-2">
										{topQueries.slice(0, 5).map((query) => (
											<div
												key={query.query}
												className="items-between flex justify-between rounded-lg bg-gray-50 p-3 dark:bg-gray-800/50"
											>
												<div className="flex-1">
													...
													<p className="text-[11px] text-gray-500 dark:text-gray-400">
														CTR: {query.ctr.toFixed(2)}% · Pos: {query.position.toFixed(1)}
													</p>
												</div>
												<div className="ml-2">
								...
								<div className="flex flex-col items-center justify-center py-10 text-center">
									...
									<p className="mt-1 max-w-sm text-[13px] text-gray-500 dark:text-gray-400">
										Per-keyword CTR trend (13-week). Status: Building / Flat / Weakening. Improving
										CTR today builds ranking power through the next 13 months.
									</p>
								</div>
							)
						) : (
							<div className="flex flex-col items-center justify-center py-10 text-center">
								<TrendingUp className="size-10 text-gray-400 dark:text-gray-500" />
								<p className="mt-3 text-sm font-medium text-gray-700 dark:text-gray-300">
									Connect Google Search Console to see CTR trends
								</p>
								<p className="mt-1 max-w-sm text-[13px] text-gray-500 dark:text-gray-400">
									Per-keyword CTR trend (13-week). Status: Building / Flat / Weakening. Improving
									CTR today builds ranking power through the next 13 months.
