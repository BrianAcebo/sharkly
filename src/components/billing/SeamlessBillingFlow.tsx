	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (setupClientSecret && !clientSecret) {
				...
				if (setupError) {
					onError(setupError.message ?? 'Unable to save payment method');
					return;
				}
			...
			<div className="space-y-6">
				...
				<div className="rounded-xl border border-emerald-100 bg-emerald-50/70 px-5 py-4 text-sm text-emerald-800 dark:border-emerald-900/60 dark:bg-emerald-900/20 dark:text-emerald-100">
					...
					<div className="mt-1 flex items-center gap-3 text-xs">
						<div className="rounded-sm border border-gray-200 px-2 py-0.5">
							<BrandIcon brand={savedPaymentMethod.brand || undefined} />
						</div>
						<p>
							{(savedPaymentMethod.brand || 'Card').toUpperCase()} ending in{' '}
...
const PlanSummary: React.FC<PlanSummaryProps> = ({ orgName, selectedPlan, trialSelected }) => (
	<div className="space-y-4 text-sm">
		...
		<Card className="border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-800">
					...
					<div>
						...
						<p className="text-sm text-gray-600 dark:text-gray-400">
							{Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(
								selectedPlan.base_price_cents / 100
							)}{' '}
							/ month
						</p>
					</div>
...
function BrandIcon({
	...
	if (b === 'mastercard' || b === 'mc' || b === 'master card')
		return <SiMastercard className={className} />;
	...
	if (b === 'diners' || b === 'diners_club' || b === 'diners club')
		return <SiDinersclub className={className} />;
	if (b === 'jcb') return <SiJcb className={className} />;
	return <CreditCardIcon className={className} />;
}

const SeamlessBillingFlow: React.FC<SeamlessBillingFlowProps> = ({
	onClose,
	existingOrganization
}) => {
	...
	const [savedPaymentMethod, setSavedPaymentMethod] = useState<CustomerPaymentMethodSummary | null>(
		null
	);
		...
		try {
			...
			if (existingOrganization?.plan_code) {
				const current = plans.find(
					(p: PlanCatalogRow) => p.plan_code === existingOrganization.plan_code
				);
	...
	const fetchDefaultPaymentMethod = async (orgId: string) => {
		try {
			...
			const response = await api.get(
				`/api/billing/orgs/payment-methods/default?${params.toString()}`,
				{
					headers: {
						Authorization: `Bearer ${session.access_token}`
					}
			...
			if (data.hasDefault) {
				...
				setSelectedSavedPaymentMethodId(
					(data.defaultPaymentMethod as CustomerPaymentMethodSummary).id
				);
			} else {
			...
			<div className="space-y-2">
				...
				<div className="grid gap-2">
					{savedPaymentMethods.map((pm) => (
						<label
							key={pm.id}
							className={`w-full cursor-pointer rounded-xl border p-4 text-left transition focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none ${
								useExistingPaymentMethod && selectedSavedPaymentMethodId === pm.id
									? 'border-blue-400 bg-blue-50 text-gray-900 shadow-sm dark:border-blue-500/60 dark:bg-blue-500/10 dark:text-gray-200'
									: 'hover-border-blue-300 border-gray-200 bg-white text-gray-700 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200'
							}`}
							onClick={() => {
						...
						>
							<div className="flex items-center justify-between">
								...
								<input
									type="radio"
									checked={useExistingPaymentMethod && selectedSavedPaymentMethodId === pm.id}
									readOnly
								/>
							</div>
							...
							<div className="flex items-center gap-3">
								...
								<div>
									...
									<p className="text-xs text-gray-600 dark:text-gray-400">
										{savedPaymentMethod.brand ? savedPaymentMethod.brand.toUpperCase() : 'Card'}{' '}
					...
					<button
						...
						className={`w-full rounded-xl border p-4 text-left transition focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none ${
							!useExistingPaymentMethod
								? 'border-blue-400 bg-blue-50 text-gray-900 shadow-sm dark:border-blue-500/60 dark:bg-blue-500/10 dark:text-gray-200'
								: 'hover-border-blue-300 border-gray-200 bg-white text-gray-700 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200'
						}`}
					>
						...
						<p className="text-xs text-gray-600 dark:text-gray-400">
							Enter new payment details to update billing
						</p>
					</button>
				</div>
			</div>
					...
					<div className="space-y-6">
						...
						{useExistingPaymentMethod && savedPaymentMethod ? (
							<ExistingPaymentMethodForm
								...
								setOrgId={setOrgId}
								savedPaymentMethod={
									savedPaymentMethods.find((m) => m.id === selectedSavedPaymentMethodId) ||
									savedPaymentMethod
								}
