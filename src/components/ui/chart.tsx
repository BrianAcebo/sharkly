...
>(
	(
		...
		const tooltipLabel = React.useMemo(() => {
			...
			if (labelFormatter) {
				return (
					<div className={cn('font-medium', labelClassName)}>{labelFormatter(String(value))}</div>
				);
			}
...
function getPayloadConfigFromPayload(config: ChartConfig, payload: unknown, key: string) {
	...
	const item = payload as { dataKey?: string; name?: string; value?: string };
	const itemKey = item.dataKey || item.name || item.value || key;
