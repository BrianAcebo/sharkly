...
export const TrialDetailsModal: React.FC<TrialDetailsModalProps> = ({
	...
	const { isOnTrial, daysRemaining, trialEndFormatted, warningLevel, organization } = trialInfo;
	...
	const getCurrentPlan = () => {
		if (organization?.plan_code) {
			const planName = organization.plan_code
				.replace('_', ' ')
		...
		const features = [
			{
				...
				text: `${organization?.included_seats || 1} team member${(organization?.included_seats || 1) > 1 ? 's' : ''}`
			},
			{
				icon: <Coins className="h-4 w-4" />,
				text: `${organization?.included_credits || 0} credits`
			}
		];
		return features;
	};
	...
	return (
		<Dialog open={isOpen} onOpenChange={onClose}>
			<DialogContent className="max-w-2xl">
				<DialogHeader>
					<DialogTitle className="flex items-center space-x-2">Trial Details</DialogTitle>
				</DialogHeader>
					...
					<Card className={getCardStyles()}>
						<CardContent className="p-4">
							<div className="space-y-3">
								<h3 className={`flex items-center gap-4 text-lg font-semibold ${getTextStyles()}`}>
									{getIcon()} {getStatusText()}
								</h3>
								<p className={`text-sm ${getTextStyles()}`}>{getStatusMessage()}</p>
								{trialEndFormatted && !needsPayment && (
									<div className="flex items-center space-x-2 text-sm">
										<Calendar className="h-4 w-4" />
										<span className={getTextStyles()}>Trial ends: {trialEndFormatted}</span>
									</div>
								)}
							</div>
						</CardContent>
