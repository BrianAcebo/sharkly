...
import { AlertTriangle, Check, Search, Plus, MapPin, XCircle, Target, HelpCircle } from 'lucide-react';
...
export default function CROStudio() {
	...
	const fetchAudits = useCallback(() => {
		...
		setLoading(true);
		fetch(`/api/cro-studio/audits?page_type=${activeTab}`, {
	...
	}, [canAccess, session?.access_token, activeTab]);
	...
	const avgScore =
		audits.length > 0
			? Math.round(
					audits.reduce((sum, a) => sum + (a.max_score > 0 ? (a.cro_score / a.max_score) * 100 : 0), 0) /
						audits.length
				)
			: 0;

	const handoffFailCount = audits.filter((a) => a.page_type === 'seo_page' && a.destination_url && !a.handoff_pass).length;
	...
	return (
		<CROStudioGate variant="main">
			<PageMeta title="CRO Studio" description="Conversion optimization for SEO and destination pages" noIndex />
			<div className="space-y-6">
				<PageHeader
					title="CRO Studio"
					subtitle="AI-powered conversion audits · Copy fixes · Persuasion analysis"
					rightContent={
						<Button onClick={() => setAddPageOpen(true)} className="bg-brand-500 hover:bg-brand-600 text-white">
				...
				<div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
					<StatCard
						...
						icon={<Target className="size-5 text-brand-500 dark:text-brand-400" />}
					/>
					<StatCard
						label="Avg CRO Score"
						...
						<div className="flex gap-1 rounded-lg border border-gray-200 bg-gray-50 p-1 dark:border-gray-700 dark:bg-gray-800/50">
							<button
								...
								className={`flex items-center gap-2 rounded-md px-4 py-2 text-sm font-medium transition-all ${
									activeTab === 'seo_page'
										? 'border-brand-500 bg-brand-50 text-brand-600 shadow-sm dark:border-brand-500 dark:bg-brand-900/30 dark:text-brand-400 border'
				...
				<div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900">
					<div className="flex items-center justify-between border-b border-gray-200 px-6 py-4 dark:border-gray-700">
						<h3 className="font-montserrat flex items-center gap-2 text-base font-bold text-gray-900 dark:text-white">
							{activeTab === 'seo_page' ? (
								<Search className="size-4 text-brand-500 dark:text-brand-400" />
							) : (
					...
					{loading ? (
						...
						<div className="flex flex-col items-center justify-center px-6 py-16 text-center">
							<div className="mb-4 flex size-16 items-center justify-center rounded-2xl bg-brand-50 dark:bg-brand-900/20">
								{activeTab === 'seo_page' ? (
									<Search className="size-8 text-brand-500 dark:text-brand-400" />
								) : (
									<MapPin className="size-8 text-brand-500 dark:text-brand-400" />
								)}
							</div>
							<h3 className="mb-2 font-montserrat text-lg font-semibold text-gray-900 dark:text-white">
								No {activeTab === 'seo_page' ? 'SEO' : 'destination'} pages yet
							</h3>
							<p className="mb-6 max-w-sm text-sm text-gray-600 dark:text-gray-400">
								Add a page to get AI-powered conversion audits, copy fixes, and persuasion analysis.
							</p>
							<Button onClick={() => setAddPageOpen(true)} className="bg-brand-500 hover:bg-brand-600">
								<Plus className="size-4" />
								Add your first page
							</Button>
						</div>
									...
									>
										<td className="px-6 py-4">
											<Link to={`/cro-studio/audit/${a.id}`} className="block">
												...
												<div className="text-xs text-gray-500 dark:text-gray-400">{a.page_url}</div>
												{a.page_type === 'seo_page' && a.destination_url && (
													<span
														className={`mt-1 inline-flex items-center gap-1 text-xs ${
															a.handoff_pass ? 'text-green-600 dark:text-green-400' : 'text-amber-600 dark:text-amber-400'
														}`}
													>
														{a.handoff_pass ? <Check className="size-3" /> : <XCircle className="size-3" />}
			...
			<Dialog open={diffModalOpen} onOpenChange={setDiffModalOpen}>
				<DialogContent className="max-w-lg">
					...
					<div className="space-y-4 text-sm text-gray-700 dark:text-gray-300">
						<p>
							<strong>SEO Pages</strong> are your supporting articles and focus page — the pages built to rank on Google. They need strong content depth, keyword structure, and internal linking. Their CRO job is light: keep the reader engaged, match their intent, and point them toward the next step at the right moment. Pushing too hard to sell here actively hurts your ranking.
						</p>
						<p>
							<strong>Destination Pages</strong> are where the sale happens. They don&apos;t need to rank — your SEO pages do that work and send qualified visitors here. So instead of optimising for Google, these pages are optimised purely for conversion: clear headline, strong trust signals, no friction between the visitor and the action you want them to take.
						</p>
						<p>
							That&apos;s why we audit them differently. Applying SEO rules to a destination page, or conversion pressure to an SEO page, makes both perform worse. Each page has one job — we check how well it&apos;s doing that job.
						</p>
					</div>
				</DialogContent>
			</Dialog>
