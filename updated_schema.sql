...
-- Team Members policies (replaces investigators policies)
...
-- Organization invites policies (updated to use team_members)
...
-- Function to create organization with admin (updated to use team_members)
...
-- (This assumes you have a user_organizations table for the junction)
...
-- SELECT 
--     i.profile_id as user_id,
--     i.organization_id,
--     CASE 
--         WHEN i.role = 'admin' THEN 'admin'
--         WHEN i.role = 'analyst' THEN 'member'
--         WHEN i.role = 'viewer' THEN 'member'
--         ELSE 'member'
--     END as role,
--     i.created_at,
--     NOW() as updated_at
