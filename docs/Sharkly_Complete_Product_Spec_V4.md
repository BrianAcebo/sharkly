...
8. React Flow map populates with focus page node \+ 5 article nodes.
...
Quick Wins \= achievable \+ KGR \< 0.25 \+ mofu or bofu. (KGR scoring requires allintitle Serper query — see Section 17.4.)
...
## **17.4 Keyword Golden Ratio (KGR) — Quick Wins Identification**

| 📌  PATENT: SEO Dissertation Section 5.3. KGR identifies underserved keywords — low competition relative to search volume. Powers the Quick Wins filter on the Strategy page. |
| :---- |

| // Keyword Golden Ratio KGR \= allintitle\_results / monthly\_search\_volume   KGR \< 0.25  \= underserved opportunity — strong ranking potential KGR 0.25-1.0 \= moderate competition KGR \> 1.0   \= oversaturated — avoid   // Implementation: // Query Serper.dev with "allintitle:\[keyword\]" → get totalResults // Divide by monthly\_searches from standard SERP call // Store as kgr\_score on topics table   // Quick Wins Definition (ALL 3 criteria required): // 1\. authority\_fit \=== "achievable" // 2\. kgr\_score \< 0.25 // 3\. funnel\_stage \=== "mofu" or "bofu"   // Tooltip: "These keywords have high commercial value and less competition // than most. Great starting points if you're building from scratch." |
| :---- |
