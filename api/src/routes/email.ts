...
router.post('/test', async (req, res) => {
  try {
    ...
    if (!target) {
      return res.status(400).json({ error: { message: 'Provide a to email or set TEST_EMAIL_TO' } });
    }
...
router.post('/test-template', async (req, res) => {
  try {
    const { to, type } = req.body as { to?: string; type?: 'teamMemberInvite' | 'organizationDeleted' | 'trialStarted' | 'trialEndingSoon' | 'trialEndsTomorrow' | 'trialEnded' };
    ...
    const trialEndIso: string = req.body?.trialEnd || new Date(Date.now() + 3 * 24 * 60 * 60 * 1000).toISOString();
    const daysRemaining: number = typeof req.body?.daysRemaining === 'number' ? req.body.daysRemaining : 3;
