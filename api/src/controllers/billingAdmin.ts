...
// Plain text: "TrueSight2026Admin!" - SAVE THIS IN YOUR NOTES
// To generate new: echo -n "YourNewPassword" | shasum -a 256 | cut -d' ' -f1
const ADMIN_PASSWORD_HASH = '5142e0d05610d35bde8c3c3af29506e5c030cdc543d7920c10b7055842706cb7';
...
export const getRefundAudit = async (req: Request, res: Response) => {
  try {
    ...
    const { data: actionResults } = await supabase
      .from('action_results')
      .select('id, action_key, entity_type, entity_id, success, error_message, credits_charged, created_at')
      .eq('organization_id', orgId)
...
export const adminProcessWalletRefund = async (req: Request, res: Response) => {
  try {
    ...
    const walletCharge = charges.data.find(c =>
      c.status === 'succeeded' &&
      !c.refunded &&
...
export const adminCreditBackAction = async (req: Request, res: Response) => {
  try {
    ...
    if (!orgId || !actionKey || !credits || !reason) {
      return res.status(400).json({ error: 'Missing required fields: orgId, actionKey, credits, reason' });
    }
...
export const cancelSubscription = async (req: Request, res: Response) => {
  try {
    ...
    if (immediately) {
      ...
      console.log('[ADMIN] Subscription canceled immediately', { orgId, subId: org.stripe_subscription_id });
    } else {
      // Cancel at period end
      await stripe.subscriptions.update(org.stripe_subscription_id, {
        cancel_at_period_end: true
      });
      console.log('[ADMIN] Subscription set to cancel at period end', { orgId, subId: org.stripe_subscription_id });
    }
