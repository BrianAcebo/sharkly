...
export interface SeatSummaryData {
	...
	nextPlan: Pick<
		PlanCatalogRow,
		'plan_code' | 'name' | 'included_seats' | 'base_price_cents'
	> | null;
...
async function ensureSeatAssignmentsFromMembers(orgId: string) {
	const [{ data: seatRows, error: seatError }, { data: memberRows, error: memberError }] =
		await Promise.all([
			supabase.from('seats').select('id, org_id, user_id, status, created_at').eq('org_id', orgId),
	...
	if (seatsToReactivate.length > 0) {
		await supabase.from('seats').update({ status: 'active' }).in('id', seatsToReactivate);
	}
...
function calculateExtraSeatUnitPrice(
	planPriceCents: number | null,
	includedSeats: number
): number | null {
...
export async function loadSeatSummary(orgId: string): Promise<SeatSummaryData> {
	const { data: organization, error: orgError } = await supabase
		.from('organizations')
		.select('id, plan_code, plan_price_cents, included_seats, max_seats, stripe_subscription_id')
		.eq('id', orgId)
		.single();
	...
	const [
		...
		{ data: seatEvents, error: eventError },
		assignedCountResult
	] = await Promise.all([
		supabase
			.from('seats')
	...
	const seatList: (SupabaseSeatRow & {
	...
	const extraSeatCountFromSeats =
		activeSeatCount > includedSeats ? activeSeatCount - includedSeats : 0;
	...
	if (plans && plans.length > 0) {
      ...
      const env = process.env.NODE_ENV === 'production' ? 'live' : 'test';
			...
			if (addonError) {
        console.error('addonError', addonError);
				throw addonError;
			}
	...
	const extraSeatsRemainingBeforeUpgrade =
		maxExtraSeatsBeforeUpgrade !== null
			? Math.max(0, maxExtraSeatsBeforeUpgrade - extraSeatsPurchased)
			: null;
	const reachedUpgradeThreshold =
		limitBeforeUpgrade !== null ? capacity >= limitBeforeUpgrade : false;
...
export async function updateOrgMaxSeats(orgId: string, newCapacity: number) {
	await supabase.from('organizations').update({ max_seats: newCapacity }).eq('id', orgId);
}
...
export async function syncExtraSeatAddon(params: {
	...
	const existing = subscription.items.data.find(
		(item) => (item as Stripe.SubscriptionItem).price?.id === addonPriceId
